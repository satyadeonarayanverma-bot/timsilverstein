...
content = re.sub(
    r"<p>Unburdened by Legacy Tools,.*?and Talent</p>",
    new_hero_subtext,
    content,
    flags=re.DOTALL
)
print("Hero subtext replaced.")

# 2. Update Header Logo Text "TechFlow Solutions" to "Cloudiva.ai"
content = re.sub(
    r"TechFlow <span style=\"color: #0084ff([^>]*?)\">Solutions</span>",
    r"Cloudiva<span style=\"color: #33a1fd\1\">.ai</span>",
    content,
    flags=re.IGNORECASE
)
...
# 3. Overwrite the AI-Forward Cloud Services block with our dark 4-card layout
start_find = '<div class="elementor-element elementor-element-2b7c9a0'
end_find = '<section class="why-choose-section">'

start_idx = content.find(start_find)
if start_idx != -1:
    end_idx = content.find(end_find, start_idx)
    if end_idx != -1:
...
"""
        content = content[:start_idx] + new_cloud_services + "\n\t\t\t\t\t\t\t\t\t\t" + content[end_idx:]
